...
def compute_supervised_loss(predict, target, reduction=True):
    # predict: prob, target: ground truth label
    if reduction:
...
def predict_mask(logit):
    h, w, n= logit.shape
    prediction = torch.empty(h, w)

    for i in range(h):
        for j in range(w):
            # If some class logit is larger than a high threshold
            if logit[i, j].max() > 0.8: 
                prediction[i, j] = logit[i, j].argmax()
            
            # If some class logit is larger than others and the diffrence is larger than a certain threshold
            top_values, _ = torch.topk(logit[i, j], 2)

            if top_values[0] - top_values[1] > 0.3:
                prediction[i, j] = logit[i, j].argmax()
