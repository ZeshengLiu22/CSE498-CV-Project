...
The code is taken from the github and made necessary updates for training and evaluating on RescueNet and FloodNet.
Siginicant change is to parallel it with [Accelerate](https://huggingface.co/docs/accelerate/en/index).
 
No special requirements are need except for the Accelerate package.

File Description:
- nohup_floodnet_reco.log: Log file for FloodNet training
- nohup_rescuenet_log.out: Log file for RescueNet training
- train_semisup_acc.py: Training Code
- visual.py: Evaluation Code
- dataset: Folder for store all the datasets and txt files for splits. Put the corresponding files in to the right train/test/validation folder.

To reproduce the results on RescueNet and FloodNet:
```
nohup accelerate launch train_semisup_acc.py --dataset rescuenet --apply_aug classmix --apply_reco  # Reproduce RescueNet Results
nohup accelerate launch train_semisup_acc.py --dataset floodnet --apply_aug classmix --apply_reco > floodnet_reco.log 2> err.log & # Reproduce FloodNet Results
```

To generate segmentation masks on certain image:
change the image id in the visual.py file and run
```
python visual.py
```

Modified by Zesheng Liu
